...
CREATE TABLE users (
    ...
    user_type VARCHAR(50) NOT NULL DEFAULT 'patient' COMMENT 'patient, doctor, caregiver, admin',
...
CREATE TABLE patients (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    user_id BIGINT UNSIGNED COMMENT 'Link to users table if patient is also a user',
    name VARCHAR(255) NOT NULL,
    age INT,
    gender VARCHAR(20),
    phone VARCHAR(50),
    address TEXT,
    diagnosis TEXT,
    caregiver_id BIGINT UNSIGNED COMMENT 'ID of caregiver from users table',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted_at TIMESTAMP NULL,
...
-- Patient bookings with doctors
-- KEY CHANGES:
-- - patient_id links to users table (not patients table)
-- - doctor_id links to users table
-- - Only patients can book appointments
-- - Doctors and patients can view their appointments
-- ========================================
CREATE TABLE appointments (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    patient_id BIGINT UNSIGNED NOT NULL COMMENT 'Patient user ID from users table',
    doctor_id BIGINT UNSIGNED NOT NULL COMMENT 'Doctor user ID from users table',
    date DATETIME NOT NULL,
    time VARCHAR(50) NOT NULL,
    type VARCHAR(50) COMMENT 'consultation, follow-up, emergency, checkup',
    status VARCHAR(50) DEFAULT 'pending' COMMENT 'pending, confirmed, cancelled, completed',
    notes TEXT COMMENT 'Additional notes (name, phone, email from booking form)',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted_at TIMESTAMP NULL,
...
CREATE TABLE quiz_results (
    ...
    answers JSON COMMENT 'Quiz answers stored as JSON',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted_at TIMESTAMP NULL,
    INDEX idx_patient_id (patient_id),
...
CREATE TABLE contacts (
    ...
    status VARCHAR(50) DEFAULT 'new' COMMENT 'new, read, replied, closed',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted_at TIMESTAMP NULL,
    INDEX idx_status (status),
...
CREATE TABLE jobs (
    ...
    type VARCHAR(50) DEFAULT 'full-time' COMMENT 'full-time, part-time, contract, internship',
    description TEXT,
    requirements TEXT,
    salary VARCHAR(100),
    posted_by BIGINT UNSIGNED COMMENT 'User ID who posted the job',
    status VARCHAR(50) DEFAULT 'active' COMMENT 'active, closed, filled',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted_at TIMESTAMP NULL,
